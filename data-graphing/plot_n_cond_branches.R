...
my_sum <- my_sum[order(my_sum$mean),]
...
if (is.null(opt$apps) || opt$apps == "all") {
  my_subtitle = NULL #glue("All apps and average")
...
ggplot(my_sum, aes(x = reorder(App, mean), y = mean, fill = cond_bp)) +
...
comando_ejecutado <- paste("Rscript data-graphing/plot_n_cond_branches.R", paste(args_pasados, collapse = " "))
