...
# --- Check for Arguments ---
echo "This script is hardcoded to look inside 1-output-jobs/BaseCPU_speculative_experiments, because of that no argument is taken"
...
BASE_DIR=$repo_path
DATA_SRC_DIR="${BASE_DIR}/1-output-jobs/BaseCPU_speculative_experiments"
OUTPUT_DEST_DIR="${BASE_DIR}/2-parser-output"

OUTPUT_FILE="${OUTPUT_DEST_DIR}/speculative_experiments_data.csv"
...
echo "frontend_width,backend_width,commit_width,fetch_delay,general_delay,speculative,cond_bp,App,IPC,Sim_Is,total_cond_predicts,wrong_cond_predicts" > "$OUTPUT_FILE"
...
find "$DATA_SRC_DIR" -maxdepth 6 -mindepth 6 -type d | sort | while read app_dir; do
    ...
    sim_frontend_width=$(jq -r '.board.processor.cores[0].core.fetchWidth' "$config_file")
    ...
    # Extract fecth_delay
    sim_fetch_delay=$(jq -r '.board.processor.cores[0].core.fetchToDecodeDelay' "$config_file")
    # Extract general_delay
    sim_general_delay=$(jq -r '.board.processor.cores[0].core.decodeToRenameDelay' "$config_file")
    # Extract speculative
    sim_speculative=$(jq -r '.board.processor.cores[0].core.branchPred.conditionalBranchPred.speculativeHistUpdate' "$config_file")
    # Extract cond_bp
    sim_cond_bp=$(jq -r '.board.processor.cores[0].core.branchPred.conditionalBranchPred.type' "$config_file")
    if [ "$sim_cond_bp" == "LocalBP" ] && [ "$sim_speculative" == "false" ]; then
        sim_cond_bp="LocalBP_NOSPEC"
    elif [ "$sim_cond_bp" == "AlwaysBooleanBP" ]; then
        ...
        if [ "$disable_loop_pred" == "false" ] && [ "$disable_sc" == "false" ]; then
            if [ "$sim_speculative" == "false" ]; then
                sim_cond_bp="TAGE_SC_L_NOSPEC"
            else
    ...
    sim_fetch_delay=${sim_fetch_delay:-N/A}
    sim_general_delay=${sim_general_delay:-N/A}
    sim_speculative=${sim_speculative:-N/A}
    ...
    echo "${sim_frontend_width},${sim_backend_width},${sim_commit_width},${sim_fetch_delay},${sim_general_delay},${sim_speculative},${sim_cond_bp},${app_name},${sim_ipc},${sim_Is},${sim_total_cond_preds},${sim_incorrect_cond_preds}" >> "$OUTPUT_FILE"
